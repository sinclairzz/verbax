...
export function LandingMotion({ children }: { children: ReactNode }) {
  ...
  useGSAP(() => {
    ...
    media.add({
      ...
      if (reduced) return () => { delete element.dataset.motion; };
      ...
      const hero = gsap.timeline({ defaults: { duration: 0.85, ease: "power3.out" } });
      hero.from(select(".hero-kicker"), { opacity: 0, y: 16 })
        .from(select(".hero-title-line"), { opacity: 0, yPercent: 65, stagger: 0.12 }, 0.08)
        .from(select(".hero-description, .hero-method, .hero-cta, .hero-assurance"), {
          opacity: 0, y: 24, stagger: 0.1, clearProps: "opacity,transform",
        }, 0.35)
        .from(select(".preview-window"), {
          opacity: 0, y: 48, scale: 0.94, duration: 1.2, clearProps: "opacity,transform",
        }, 0.15)
        .from(select(".preview-metrics > div, .preview-row"), {
          opacity: 0, y: 12, stagger: 0.055, clearProps: "opacity,transform",
        }, 0.6)
        .from(select(".preview-float"), {
          opacity: 0, y: 28, clearProps: "opacity,transform",
        }, 0.9);
      reveals.push({ target: select(".hero")[0], animation: hero });
      if (window.scrollY > 120) hero.progress(1);

      reveal(".trust-strip > span", 0.07);
      reveal(".section-heading > *, .trust-intro > *, .product-copy > :not(.feature-list), .center-heading > *");
      ...
      reveal(".pricing-footnote, .social-placeholder > *");
      reveal(".faq-section > div:first-child > *, .faq-list details", 0.045);
      reveal(".final-cta > *, .legal-line, .footer-top > *, .footer-bottom", 0.05);
      ...
      gsap.from(select(".reference-image"), {
        ...
        scrollTrigger: {
          ...
          start: "top 90%", end: "center 55%", scrub: 0.7,
        },
      });
      gsap.from(select(".steps-rail span"), {
        ...
        scrollTrigger: {
          ...
          start: "top 88%", end: "bottom 50%", scrub: 0.5,
        },
      });
      gsap.fromTo(select(".landing-progress"), { scaleX: 0 }, {
        scaleX: 1, ease: "none",
      ...
      if (desktop) {
        gsap.to(select(".hero-visual"), {
          y: -65, ease: "none",
          scrollTrigger: { trigger: select(".hero")[0], start: "top top", end: "bottom top", scrub: 0.8 },
        });
        gsap.to(select(".preview-orbit"), {
          rotation: "+=30", scale: 1.08, ease: "none",
          scrollTrigger: { trigger: select(".hero")[0], start: "top top", end: "bottom top", scrub: 1 },
        });
        gsap.fromTo(select(".landing-glow"), { yPercent: -12 }, {
      ...
      const onFocus = (event: FocusEvent) => {
        ...
        reveals.forEach(({ target, animation }) => {
          if (target.contains(event.target as Element)) animation.progress(1);
        });
      };
